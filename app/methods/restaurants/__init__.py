from .createRestaurant import *
from .editRestaurant import *
from .isExistedRestaurant import isExistedRestaurant
from .getRestaurantViaLink import getRestaurantViaLink
from .changeTurnOfRestaurant import changeTurnOfRestaurant
from .deleteRestaurant import deleteRestaurant
from .setBannerToRestaurant import setBannerToRestaurant
from .deleteBannerToRestaurant import deleteBannerToRestaurant

from .changeTelegramAdmin import changeTelegramAdmin
